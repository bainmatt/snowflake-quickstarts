...
def get_feature_ranges(table_name):
    """
    Gets the min and max for numerical columns and distinct values for
    categorical columns.
    """
...
try:
    ...
    carat = st.sidebar.slider(
        "Carat", feature_ranges['CARAT'][0],
        feature_ranges['CARAT'][1],
        float(
            feature_ranges['CARAT'][0] +
            feature_ranges['CARAT'][1]
        ) / 2
    )
    depth = st.sidebar.slider(
        "Depth", feature_ranges['DEPTH'][0],
        feature_ranges['DEPTH'][1],
        float(
            feature_ranges['DEPTH'][0] +
            feature_ranges['DEPTH'][1]
        ) / 2)
    table = st.sidebar.slider(
        "TABLE_PCT",
        feature_ranges['TABLE_PCT'][0],
        feature_ranges['TABLE_PCT'][1],
        float(
            feature_ranges['TABLE_PCT'][0] +
            feature_ranges['TABLE_PCT'][1]
        ) / 2)
