"""
Snowflake demo: Streamlit dashboard for our diamond price regression model.

Run with:
streamlit run getting-started-with-ml-development/scripts/streamlit/diamonds_pred_app.py
"""
import os
import joblib
...
st.write(
    "This application predicts the price of a diamond "
    "based on its characteristics. "
    "Use the sliders and dropdowns below to enter the diamond's features "
    "and click 'Predict Price' to see the estimated value."
)
...
# This assumes the user has run the setup.sql and notebooks from the repo
DB_NAME = 'ML_HOL_DB'
...
def load_pipeline() -> Pipeline:
    """
    Download the preprocessing pipeline file from stage and loads into memory.
    """
    try:
        ...
        st.error(
            "Failed to load preprocessing pipeline from stage "
            f"'@{STAGE_NAME}'."
        )
        st.error(
            f"Please ensure '{PIPELINE_FILE_NAME}' exists in the stage. "
            "Error: {e}"
        )
        return None
...
# Define the logical order for categorical features to be used in UI
# and for encoding.
...
def get_feature_ranges(
    ...
    """
    Map each feature name to either a numerical range or tuple of categories.

    Returns the min and max for numerical columns and distinct values for
    categorical columns.
    """
    # User should input raw features; the pipeline will create derived
    # features like TABLE_PCT.
    ...
    for feature in numerical_features:
        result_row = df.agg(
            min(col(feature)).alias("MIN_VAL"),
            max(col(feature)).alias("MAX_VAL")
        ).collect()[0]

        min_val = result_row["MIN_VAL"]
        max_val = result_row["MAX_VAL"]
        feature_ranges[feature] = (
            float(min_val), float(max_val)  # type: ignore
        )
...
def create_slider(name: str, colname: str):
    """
    Create a slider for a numerical feature over a min/max range.
    """
    return st.sidebar.slider(
        name,
        feature_ranges[colname][0],
        feature_ranges[colname][1],
        float(
            feature_ranges[colname][0] +
            feature_ranges[colname][1]
        ) / 2
...
try:
    ...
    carat = create_slider('Carat', 'CARAT')
    depth = create_slider('Depth', 'DEPTH')
    table = create_slider('TABLE_PCT', 'TABLE_PCT')
    x = create_slider('X (Length in mm)', 'X')
    y = create_slider('Y (Width in mm)', 'Y')
    z = create_slider('Z (Depth in mm)', 'Z')
    ...
    st.error(
        "Could not load feature ranges. Make sure the table "
        f"'{TRAINING_TABLE}' exists in '{DB_NAME}.{SCHEMA_NAME}'. "
        f"Error: {e}"
    )
    st.stop()
...
if st.sidebar.button("Predict Price", type="primary"):
    with st.spinner("Preprocessing input and predicting price..."):
        try:
            ...
            # 1. Create a pandas df from user inputs with the raw feature names
            ...
            # NOTE: Emits UserWarning:
            # X has feature names, but ... was fitted without feature names
            # because the DataFrame on which we trained the preprocessing
            # pipeline was loaded as a Snowpark, rather than Pandas, DataFrame:
            # diamonds_df = session.table("DIAMONDS")
            # which is converted to a numpy array before training. To resolve,
            # could maybe use raw_features_df.values.

            # 2. Run the raw features through the loaded preprocessing pipeline
            processed_features_df = preprocessing_pipeline.fit(
                raw_features_df
            ).transform(raw_features_df)

            # 3. Get the processed feature values for the SQL query
            sql_column_order = [
                'CUT_OE', 'COLOR_OE', 'CLARITY_OE', 'CARAT', 'DEPTH',
                'TABLE_PCT', 'X', 'Y', 'Z'
            ]
            feature_values = (
                processed_features_df[sql_column_order].iloc[0]
            ).values.tolist()
            values_str = ', '.join(map(str, feature_values))

            # --- Prediction ---

            # Construct query to call the prediction model from the registry.
            # The model expects features in the order output by the pipeline.
            prediction_sql = f"""
                WITH model_version_alias AS MODEL {MODEL_NAME}
            ...
            st.metric(
                ...
                f"${float(predicted_price):,.2f}"  # type: ignore
            )
            ...
            # FIX: The column name 'TABLE' is a reserved SQL keyword and
            # must be quoted.
            ...
            # Prepare for charting using raw user inputs for interpretability
            user_input_data = {
            ...
            st.error(
                ...
                f"Please ensure the model '{MODEL_NAME}' "
                f"version '{MODEL_VERSION}' is in the Model Registry "
                f"and the pipeline file is in stage '@{STAGE_NAME}'. "
                f"Error: {e}"
            )
...
st.sidebar.info(
    """
    **Setup Instructions:**
    1. Run the `setup.sql` script from the
       [GitHub repository](https://github.com/Snowflake-Labs/
       sfguide-intro-to-machine-learning-with-snowflake-ml-for-python) to
       create the database, schema, and tables.
    2. Run through the notebooks in the repository to train the model,
       create the preprocessing pipeline, and deploy them.
    3. Ensure this Streamlit app is running in a Snowflake environment with
       access to the created database and schema.
    """
)
